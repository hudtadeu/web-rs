...
import {
  ...
  TableContainer, TableHead, TableRow, MenuItem, FormControlLabel, IconButton
} from "@mui/material";
...
const Inspecoes = () => {
  ...
  const [form, setForm] = useState({
    type: '', verificacoes: [], catRisco: [], obsVerif: [], anexoVerif: [],
    acaoImediata: '', desvios: [], participantes: []
  });
  ...
  const handleVerifChange = (i, opt) => {
    const v = [...form.verificacoes]; v[i] = v[i] === opt ? null : opt;
    setForm(f => ({ ...f, verificacoes: v }));
  };
  const handleNested = (field, i, val) => {
    const a = [...form[field]]; a[i] = val;
    setForm(f => ({ ...f, [field]: a }));
  };
  ...
  // Desvios
  const addDesvio = () => setForm(f => ({ ...f, desvios: [...f.desvios, { desvio: '', categoria: '', acoes: [], responsavel: '', prazo: '', alerta: false }] }));
  const changeDesvio = (idx, key, val) => {
    const d = [...form.desvios]; d[idx][key] = val;
    setForm(f => ({ ...f, desvios: d }));
  };
  const toggleAcao = (idx, tipo) => {
    const d = [...form.desvios];
    const list = d[idx].acoes;
    d[idx].acoes = list.includes(tipo) ? list.filter(x => x !== tipo) : [...list, tipo];
    setForm(f => ({ ...f, desvios: d }));
  };
  const toggleAlerta = idx => changeDesvio(idx, 'alerta', !form.desvios[idx].alerta);
  const removeDesvio = idx => setForm(f => ({ ...f, desvios: f.desvios.filter((_, i) => i !== idx) }));

  // Participantes
  const addPart = () => setForm(f => ({ ...f, participantes: [...f.participantes, { nome: '', funcao: '' }] }));
  const changePart = (idx, key, val) => {
    const p = [...form.participantes]; p[idx][key] = val;
    setForm(f => ({ ...f, participantes: p }));
  };
  const removePart = idx => setForm(f => ({ ...f, participantes: f.participantes.filter((_, i) => i !== idx) }));
  ...
  const save = () => {
    ...
    setInspecoes(prev => [newInspecao, ...prev]);
    const actions = form.verificacoes.map((resp, idx) => ({ pergunta: templates[form.type].verificacoes[idx], resposta: resp })).filter(item => item.resposta === 'NC');
  ...
  return (
    <Box sx={{ p: 3, maxWidth: 1400, mx: 'auto' }}>
      ...
      <Button variant="contained" startIcon={<Add />} onClick={() => setOpen(true)} sx={{ my: 2 }}>Nova Inspeção</Button>
      ...
      <Dialog open={open} onClose={() => { reset(); setOpen(false); }} fullWidth maxWidth="lg">
        ...
        <DialogContent>
          <Grid container spacing={2} sx={{ mb: 2 }}>
            ...
            <Grid item xs={6} sm={4}>
              <TextField label="Ação Imediata" fullWidth multiline rows={2}
            ...
            <>
                  ...
                  <TableBody>
                    {questions.map((q, i) => (
                      <TableRow key={i} hover>
                        ...
                        <TableCell>
                          <TextField size="small" fullWidth placeholder="Categoria"
                            value={form.catRisco[i] || ''}
                            onChange={e => handleNested('catRisco', i, e.target.value)}
                          />
                        </TableCell>
                        <TableCell>
                          <TextField size="small" fullWidth placeholder="Observações"
                            value={form.obsVerif[i] || ''}
              ...
              {form.desvios.map((d, j) => (
                <Grid container spacing={1} alignItems="center" key={j} sx={{ mb: 1 }}>
                  <Grid item xs={3}><TextField label="Desvio" size="small" fullWidth
                    value={d.desvio} onChange={e => changeDesvio(j, 'desvio', e.target.value)} /></Grid>
                  <Grid item xs={2}><TextField label="Categoria" size="small" fullWidth
                    value={d.categoria} onChange={e => changeDesvio(j, 'categoria', e.target.value)} /></Grid>
                  ...
                  <Grid item xs={2}><TextField label="Responsável" size="small" fullWidth
                    value={d.responsavel} onChange={e => changeDesvio(j, 'responsavel', e.target.value)} /></Grid>
                  <Grid item xs={2}><TextField label="Prazo" type="date" size="small" fullWidth InputLabelProps={{ shrink: true }}
                    value={d.prazo} onChange={e => changeDesvio(j, 'prazo', e.target.value)} /></Grid>
                  <Grid item xs={1} align="center"><Checkbox checked={d.alerta} onChange={() => toggleAlerta(j)} /></Grid>
                  <Grid item xs={1}><IconButton color="error" onClick={() => removeDesvio(j)}><Delete /></IconButton></Grid>
                </Grid>
              ))}
              ...
              {form.participantes.map((p, k) => (
                <Grid container spacing={1} key={k} sx={{ mb: 1 }}>
                  <Grid item xs={5}><TextField label="Nome" size="small" fullWidth
                    value={p.nome} onChange={e => changePart(k, 'nome', e.target.value)} /></Grid>
                  <Grid item xs={5}><TextField label="Função" size="small" fullWidth
                    value={p.funcao} onChange={e => changePart(k, 'funcao', e.target.value)} /></Grid>
                  <Grid item xs={2}><IconButton color="error" onClick={() => removePart(k)}><Delete /></IconButton></Grid>
                </Grid>
              ))}
